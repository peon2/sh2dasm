...
static UINT32 op1000(char *buffer, UINT32 pc, UINT16 opcode)
{
	...
	case  8:
		...
		break;
	case  9:
		sprintf(buffer, "BT      $%08lx", pc + SIGNX8(opcode & 0xff) * 2 + 2);
		break;
	case 11:
		sprintf(buffer, "BF      $%08lx", pc + SIGNX8(opcode & 0xff) * 2 + 2);
		break;
	case 13:
		sprintf(buffer, "BTS     $%08lx", pc + SIGNX8(opcode & 0xff) * 2 + 2);
		break;
	case 15:
		sprintf(buffer, "BFS     $%08lx", pc + SIGNX8(opcode & 0xff) * 2 + 2);
		break;
	default :
...
{
	sprintf(buffer, "BRA     $%08lx", SIGNX12(opcode & 0xfff) * 2 + pc + 2);
	return 0;
}

static UINT32 op1011(char *buffer, UINT32 pc, UINT16 opcode)
{
	sprintf(buffer, "BSR     $%08lx", SIGNX12(opcode & 0xfff) * 2 + pc + 2);
	return 0;
}
